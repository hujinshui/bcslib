...
BCS_TEST_CASE( test_arbb_clone )
{
	...
	caview1d<double> s1(src, N);
	arbb::dense<f64, 1> a1 = to_arbb(s1);
	array1d<double> t1 = from_arbb(a1);
	...
	caview2d<double, row_major_t> s2(src, m, n);
	arbb::dense<f64, 2> a2 = to_arbb(s2);
	array2d<double, row_major_t> t2 = from_arbb(a2, row_major_t());
	...
	caview2d<double, column_major_t> s3(src, m, n);
	arbb::dense<f64, 2> a3 = to_arbb(s3);
	array2d<double, column_major_t> t3 = from_arbb(a3, column_major_t());

	BCS_CHECK( array_view_equal(t3, src, m, n) );
}


BCS_TEST_CASE( test_arbb_bind )
{
	using arbb::f64;
	...
	aview2d<double, row_major_t> s2(src, m, n);
	arbb::dense<f64, 2> a2;
	bind(a2, s2);
	array2d<double, row_major_t> t2 = from_arbb(a2, row_major_t());
	...
	aview2d<double, column_major_t> s3(src, m, n);
	arbb::dense<f64, 2> a3;
	bind(a3, s3);
	array2d<double, column_major_t> t3 = from_arbb(a3, column_major_t());
