...
bool test_image_view(const_imview<TPixel> im, size_t w, size_t h, size_t s, const TPixel *src)
{
...
BCS_TEST_CASE( test_view_ch1 )
{
	...
	BCS_CHECK( test_image_view(im1, 3, 2, 3 * sizeof(uint8_t), srcs) );

	imview<uint8_t> im2(srcs, 3, 2, 4 * sizeof(uint8_t));
	uint8_t r2[] = {1, 2, 3, 5, 6, 7};
	BCS_CHECK( test_image_view(im2, 3, 2, 4 * sizeof(uint8_t), r2) );
}
...
BCS_TEST_CASE( test_view_ch2 )
{
	...
	BCS_CHECK( test_image_view(im1, 3, 2, 3 * sizeof(pix_t), srcs) );

	imview<pix_t> im2(srcs, 3, 2, 4 * sizeof(pix_t));
	uint8_t r2i[] = {1, 2, 3, 4, 5, 6, 9, 10, 11, 12, 13, 14};
	BCS_CHECK( test_image_view(im2, 3, 2, 4 * sizeof(pix_t), (const pix_t*)r2i) );
}
