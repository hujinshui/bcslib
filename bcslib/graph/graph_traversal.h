...
namespace bcs
{
	...
	public:
		...
		void add_seed(const vertex_type& v, Visitor& visitor)
		{
			...
			{
				visitor.seed(v);
				m_status[v.index()] = GVISIT_DISCOVERED;
	...
	private:
		...
		void add_neighbors(const vertex_type& v, Visitor& visitor)
		{
			...
			for (neighbor_iterator it = nb_begin; it != nb_end; ++it)
			{
				...
				{
					m_status[u.index()] = GVISIT_DISCOVERED;
					visitor.discover(v, u);
					m_queue.push(u);
				}
			}

			m_status[v.index()] = GVISIT_FINISHED;
			visitor.finish(v);
		}

	private:
		const IGraphAdjacencyList<Derived>& m_graph;
		array1d<gvisit_status> m_status;
	...
	public:
		...
		void next(Visitor& visitor)
		{
			...
			{
				m_status[e.v.index()] = GVISIT_FINISHED;
				visitor.finish(e.v);
				m_stack.pop();
