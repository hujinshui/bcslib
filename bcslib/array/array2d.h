...
namespace bcs
{
	...
	template<typename T, typename TOrd, class TIndexer0=id_ind, class TIndexer1=id_ind> class caview2d;
	...
	namespace _detail
	{
		...
		public:
			typedef T value_type;
			typedef caview2d<T, TOrd, TIndexer0, TIndexer1> cview_type;

			explicit _arr2d_transpose_t(const cview_type& a) : m_cref(a) { }
	...
	public:
		...
		typedef caview2d<value_type, layout_order, indexer0_type, indexer1_type> cview_type;
	...
	public:
		caview2d(const_pointer pbase, index_type base_d0, index_type base_d1,
	...
	public:
		caview2d(const_pointer pbase, const _index_core_type& idxcore)
	...
	public:
		...
		caview1d<value_type, slice0_indexer_type> sliceI0(index_type i) const
		{
			return caview1d<value_type, slice0_indexer_type>(
					m_pbase + m_idxcore.sliceI0_offset(i), m_idxcore.sliceI0_indexer());
		}

		caview1d<value_type, slice1_indexer_type> sliceI1(index_type j) const
		{
			return caview1d<value_type, slice1_indexer_type>(
					m_pbase + m_idxcore.sliceI1_offset(j), m_idxcore.sliceI1_indexer());
		}

		caview1d<value_type, slice0_indexer_type> row(index_type i) const
		{
			return sliceI0(i);
		}

		caview1d<value_type, slice1_indexer_type> column(index_type j) const
		{
			return sliceI1(j);
		}
		...
		caview1d<value_type, typename sub_indexer<slice0_indexer_type, TSelector>::type>
		...
		template<class TSelector>
		caview1d<value_type, typename sub_indexer<slice1_indexer_type, TSelector>::type>
		...
		template<class TSelector0, class TSelector1>
		caview2d<value_type, layout_order,
		...
		V(const TSelector0& sel0, const TSelector1& sel1) const
		{
			...
			return caview2d<value_type, layout_order, sub_indexer0_t, sub_indexer1_t>(
	...
	template<typename T, typename TOrd, class TIndexer0, class TIndexer1>
	class aview2d : public caview2d<T, TOrd, TIndexer0, TIndexer1>
	{
	public:
		BCS_ARRAY_CHECK_TYPE(T)
		BCS_ARRAY_BASIC_TYPEDEFS(2u, T, TOrd)
	...
	public:
		...
		caview1d<value_type, slice0_indexer_type> sliceI0(index_type i) const
		{
			return aview1d<value_type, slice0_indexer_type>(
					this->m_pbase + this->m_idxcore.sliceI0_offset(i), this->m_idxcore.sliceI0_indexer());
		}

		caview1d<value_type, slice1_indexer_type> sliceI1(index_type j) const
		{
			return aview1d<value_type, slice1_indexer_type>(
					this->m_pbase + this->m_idxcore.sliceI1_offset(j), this->m_idxcore.sliceI1_indexer());
		}
		...
		V(const TSelector0& sel0, const TSelector1& sel1) const
		{
			...
			index_t offset;
			auto sub_idxcore = this->m_idxcore.subview_index_core(sel0, sel1, offset);

			return aview2d<value_type, layout_order, sub_indexer0_t, sub_indexer1_t>(
					this->m_pbase + offset, std::move(sub_idxcore));
		}
	...
	{
		return caview2d<T, row_major_t>(base, (index_t)m, (index_t)n, m, n);
	}

	template<typename T>
	inline aview2d<T, row_major_t> get_aview2d_rm(T *base, size_t m, size_t n)
	{
		return aview2d<T, row_major_t>(base, (index_t)m, (index_t)n, m, n);
	}

	template<typename T>
	inline caview2d<T, column_major_t> get_aview2d_cm(const T *base, size_t m, size_t n)
	{
		return caview2d<T, column_major_t>(base, (index_t)m, (index_t)n, m, n);
	}

	template<typename T>
	inline aview2d<T, column_major_t> get_aview2d_cm(T *base, size_t m, size_t n)
	{
		return aview2d<T, column_major_t>(base, (index_t)m, (index_t)n, m, n);
	}

	template<typename T, class TIndexer0, class TIndexer1>
	inline caview2d<T, row_major_t, TIndexer0, TIndexer1> get_aview2d_rm_ex(const T *base, size_t base_m, size_t base_n,
			const TIndexer0& idxer0, const TIndexer1& idxer1)
	{
		return caview2d<T, row_major_t, TIndexer0, TIndexer1>(base, (index_t)base_m, (index_t)base_n, idxer0, idxer1);
	}

	template<typename T, class TIndexer0, class TIndexer1>
	inline aview2d<T, row_major_t, TIndexer0, TIndexer1> get_aview2d_rm_ex(T *base, size_t base_m, size_t base_n,
			const TIndexer0& idxer0, const TIndexer1& idxer1)
	{
		return aview2d<T, row_major_t, TIndexer0, TIndexer1>(base, (index_t)base_m, (index_t)base_n, idxer0, idxer1);
	}

	template<typename T, class TIndexer0, class TIndexer1>
	inline caview2d<T, column_major_t, TIndexer0, TIndexer1> get_aview2d_cm_ex(const T *base, size_t base_m, size_t base_n,
			const TIndexer0& idxer0, const TIndexer1& idxer1)
	{
		return caview2d<T, column_major_t, TIndexer0, TIndexer1>(base, (index_t)base_m, (index_t)base_n, idxer0, idxer1);
	}

	template<typename T, class TIndexer0, class TIndexer1>
	inline aview2d<T, column_major_t, TIndexer0, TIndexer1> get_aview2d_cm_ex(T *base, size_t base_m, size_t base_n,
			const TIndexer0& idxer0, const TIndexer1& idxer1)
	{
		return aview2d<T, column_major_t, TIndexer0, TIndexer1>(base, (index_t)base_m, (index_t)base_n, idxer0, idxer1);
	}
	...
	template<typename T, typename TOrd, class TIndexer0, class TIndexer1>
	struct is_array_view<bcs::caview2d<T, TOrd, TIndexer0, TIndexer1> > : public std::true_type { };

	template<typename T, typename TOrd, class TIndexer0, class TIndexer1>
	struct is_array_view<bcs::aview2d<T, TOrd, TIndexer0, TIndexer1> > : public std::true_type { };

	template<typename T, typename TOrd, class Alloc>
	struct is_array_view<bcs::array2d<T, TOrd, Alloc> > : public std::true_type { };

	template<typename T, typename TOrd, class TIndexer0, class TIndexer1>
	struct is_array_view_ndim<bcs::caview2d<T, TOrd, TIndexer0, TIndexer1>, 2> : public std::true_type { };

	template<typename T, typename TOrd, class TIndexer0, class TIndexer1>
	struct is_array_view_ndim<bcs::aview2d<T, TOrd, TIndexer0, TIndexer1>, 2> : public std::true_type { };

	template<typename T, typename TOrd, class Alloc>
	struct is_array_view_ndim<bcs::array2d<T, TOrd, Alloc>, 2> : public std::true_type { };


	template<typename T, typename TOrd, class TIndexer0, class TIndexer1>
	inline std::array<index_t, 2> get_array_shape(const bcs::caview2d<T, TOrd, TIndexer0, TIndexer1>& arr)
	{
		return arr.shape();
	}

	template<typename T, typename TOrd, class TIndexer0, class TIndexer1>
	inline size_t get_num_elems(const bcs::caview2d<T, TOrd, TIndexer0, TIndexer1>& arr)
	{
		return arr.nelems();
	}

	template<typename T, typename TOrd, class TIndexer0, class TIndexer1>
	inline const T& get(const bcs::caview2d<T, TOrd, TIndexer0, TIndexer1>& arr, index_t i, index_t j)
	{
		return arr(i, j);
	}
	...
	begin(const bcs::caview2d<T, TOrd, TIndexer0, TIndexer1>& arr)
	{
		return arr.begin();
	}
	...
	template<typename T, typename TOrd, class TIndexer0, class TIndexer1>
	inline typename bcs::aview2d<T, TOrd, TIndexer0, TIndexer1>::const_iterator
	end(const bcs::caview2d<T, TOrd, TIndexer0, TIndexer1>& arr)
	{
		return arr.end();
	}
	...
	template<typename T, typename TOrd, class TIndexer0, class TIndexer1>
	inline const T* ptr_base(const bcs::caview2d<T, TOrd, TIndexer0, TIndexer1>& arr)
	{
		return arr.pbase();
	}
	...
	template<typename T, class TIndexer0, class TIndexer1>
	inline bool is_dense_view(const caview2d<T, row_major_t, TIndexer0, TIndexer1>& view)
	{
	...
	template<typename T, class TIndexer0, class TIndexer1>
	inline bool is_dense_view(const caview2d<T, column_major_t, TIndexer0, TIndexer1>& view)
	{
	...
	template<typename T, typename TOrd, class TIndexer0, class TIndexer1>
	inline array2d<T, TOrd> clone_array(const caview2d<T, TOrd, TIndexer0, TIndexer1>& view)
	{
	...
	operator >> (const caview2d<T, TOrd, LIndexer0, LIndexer1>& a, aview2d<T, TOrd, RIndexer0, RIndexer1>& b)
	{
	...
	operator << (aview2d<T, TOrd, LIndexer0, LIndexer1>& a, const caview2d<T, TOrd, RIndexer0, RIndexer1>& b)
	{
		if (get_num_elems(a) != get_num_elems(b))
		{
			throw array_dim_mismatch();
	...
	namespace _detail
	{
		...
		struct _array_creater_base<T, 2, TOrd>
		{
			...
			struct remap
			{
				typedef _detail::_array_creater_base<U, D, TOrd> type;
			};
		};
	}

	template<typename T, typename TOrd, class TIndexer0, class TIndexer1>
	struct array_creater<caview2d<T, TOrd, TIndexer0, TIndexer1> > : public _detail::_array_creater_base<T, 2, TOrd> { };

	template<typename T, typename TOrd, class TIndexer0, class TIndexer1>
	struct array_creater<aview2d<T, TOrd, TIndexer0, TIndexer1> > : public _detail::_array_creater_base<T, 2, TOrd> { };

	template<typename T, typename TOrd, class Alloc>
	struct array_creater<caview2d<T, TOrd, Alloc> > : public _detail::_array_creater_base<T, 2, TOrd> { };
